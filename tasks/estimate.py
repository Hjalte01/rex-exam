...
class Estimate(Task):
    def __init__(self, aruco_dict, marker_size, cam_matrix, dist_coeffs, initial_control: Liste[float, float], grid: Grid):
        super().__init__()
    ...
    def run(self, robot: ExamRobot):
        ...
        if ids is None or all(len(x) >= 2 for x in ids): # Python might not shortcircuit
            return
